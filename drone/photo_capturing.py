...
def avoid_object(tello):
    ...
    Maneuver sequence:
      1. Stop.
      2. Move right 75 cm to clear the object's path.
...
def fly_segment(tello, net, output_layers, classes,
    ...
    while time.time() - start_time < desired_time:
        ...
        # Capture and save a photo every photo_interval seconds.
...
def fly_square_path(tello, net, output_layers, classes):
    """
    Flies a 2x2 m square around an object.
    
    Assuming the drone starts at the northwest corner, the segments are defined as follows:
    
      Segment 1 (North edge, 200 cm): 
          - Check yaw = 90° (east) 
          - Flight yaw = 0° (camera faces inward toward the center)
      
      Segment 2 (East edge, 200 cm): 
          - Check yaw = 180° (south) 
          - Flight yaw = 270° (camera faces inward)
      
      Segment 3 (South edge, 200 cm): 
          - Check yaw = 270° (west) 
          - Flight yaw = 180° (camera faces inward)
      
      Segment 4 (West edge, 200 cm): 
          - Check yaw = 0° (north) 
          - Flight yaw = 90° (camera faces inward)
    """
    # Segment 1
    ...
    # Segment 2
    fly_segment(tello, net, output_layers, classes,
                distance=200, check_yaw=180, flight_yaw=270,
                forward_speed=20, check_interval=0.1, photo_interval=1.0)
    # Segment 3
    fly_segment(tello, net, output_layers, classes,
    ...
    # Segment 4
    fly_segment(tello, net, output_layers, classes,
                distance=200, check_yaw=0, flight_yaw=90,
                forward_speed=20, check_interval=0.1, photo_interval=1.0)
