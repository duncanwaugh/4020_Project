...
def load_yolo():
    ...
    """
    weights_path = r"C:\Users\james\OneDrive\Desktop\4020 Project\yolov4-p6.weights"
    config_path  = r"C:\Users\james\OneDrive\Desktop\4020 Project\yolov4.cfg"
    names_path   = r"C:\Users\james\OneDrive\Desktop\4020 Project\coco.names"
...
def fly_segment(tello, net, output_layers, classes,
                ...
                forward_speed=20, check_interval=0.1, photo_interval=2.0):
    """
    Flies one segment of the path while capturing a photo every 2 seconds.
    ...
    Steps:
      ...
      5. Capture and save a photo every 'photo_interval' seconds.
    """
    # Stop movement before checking
    tello.send_rc_control(0, 0, 0, 0)
    time.sleep(0.5)

    # Rotate to check direction
    print(f"Rotating to check direction: {check_yaw}°")
    set_yaw(tello, check_yaw)
    time.sleep(1)

    # Obstacle detection
    print("Checking path for obstacles...")
    if check_for_obstacle_fused(tello, net, output_layers, classes,
                                num_checks=5, detection_threshold=3, tof_threshold=150):
        avoid_object(tello)  # Execute avoidance maneuver

    # Rotate to flight yaw
    print(f"Rotating to flight direction: {flight_yaw}°")
    ...
    last_photo_time = start_time  # Track last photo timestamp
    photo_counter = 1  # Counter for image filenames

    print(f"Flying segment: {distance} cm at {forward_speed} cm/s (~{desired_time:.1f} s).")

    while time.time() - start_time < desired_time:
        tello.send_rc_control(20, 0, 0, 0)  # Move forward
        ...
        if frame is not None:
            ...
            if current_time - last_photo_time >= photo_interval:
                timestamp = int(current_time)  # Unique timestamp
                photo_filename = os.path.join(PHOTO_DIR, f"photo_{timestamp}.jpg")
                cv2.imwrite(photo_filename, frame)  # Save image
                print(f"Photo saved: {photo_filename}")
                last_photo_time = current_time  # Update last photo time
                photo_counter += 1

        time.sleep(check_interval)  # Small delay for control loop
...
def fly_square_path(tello, net, output_layers, classes):
    ...
    fly_segment(tello, net, output_layers, classes,
                distance=500, check_yaw=90, flight_yaw=0,
                forward_speed=20, check_interval=0.1, photo_interval=2.0)
    # Segment 2
    fly_segment(tello, net, output_layers, classes,
                distance=300, check_yaw=0, flight_yaw=270,
                forward_speed=20, check_interval=0.1, photo_interval=2.0)
    # Segment 3
    fly_segment(tello, net, output_layers, classes,
                distance=500, check_yaw=270, flight_yaw=180,
                forward_speed=20, check_interval=0.1, photo_interval=2.0)
    # Segment 4
    fly_segment(tello, net, output_layers, classes,
                distance=300, check_yaw=90, flight_yaw=90,
                forward_speed=20, check_interval=0.1, photo_interval=2.0)
